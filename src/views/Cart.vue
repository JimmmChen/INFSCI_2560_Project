              ...
              <li v-for="item in cartList" :key="item.productId">
                <div class="cart-tab-1">
                  <div class="cart-item-check">
                    <a href="javascipt:;" class="checkbox-btn item-check-btn" :class="{'check':item.checked==='1'}" @click="checkToggle(item.productId, item.checked)">
                  ...
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="select-self-area">
                        <a class="input-sub" @click="editCartNum(item, -1)">-</a>
                        <span class="select-ipt">{{item.productNum}}</span>
                        <a class="input-add" @click="editCartNum(item, 1)">+</a>
                      </div>
                    </div>
                  </div>
                </div>
...
export default{
  ...
  computed: {
    checkoutDisEnable () {
      return this.checkCartItemStatus('1')
    },
    checkAllFlag () {
      return this.checkCartItemStatus('0')
    },
    totalPrice () {
      let sum = 0
  ...
  methods: {
    ...
    delCartConfirm (productId, productNum) {
      this.productIdDel = productId
      this.productNumDel = productNum
      this.modalConfirm = true
    },
    delCart () {
      axios.post('/users/cart/del', {
        productId: this.productIdDel
      }).then((response) => {
        let res = response.data
        if (res.status === '0') {
      ...
      this.$store.commit('updateCartCount', -this.productNumDel)
    },
    editCartNum (item, num) {
      let newproductNum = parseInt(item.productNum) + num
      if (newproductNum < 1 || newproductNum > 99) {
        return
      }
      axios.post('/users/cart/editNum', {
    ...
    checkToggle (productId, curChecked) {
      axios.post('/users/cart/checkToggle', {
        productId: productId,
        check: curChecked === '1' ? '0' : '1'
      }).then((response) => {
        let res = response.data
        if (res.status === '0') {
          this.init()
    ...
    toggleCheckAll () {
      axios.post('/users/cart/checkToggle', {
        checkAll: this.checkAllFlag ? '0' : '1'
      }).then((response) => {
        let res = response.data
        if (res.status === '0') {
          this.init()
